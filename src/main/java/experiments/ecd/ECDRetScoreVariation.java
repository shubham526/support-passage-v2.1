...
/**
 * Variation on the ECDRetScore method.
 * Score of passage = score of ECD in which it appears + score of all entities in the passage.
 * Entities in passage ranked by frequency of co-occurrence with target entity.
 *
 * @author Shubham Chaterjee
 * @version 7/16/2020
 */

public class ECDRetScoreVariation {
    ...
    public ECDRetScoreVariation(String indexDir,
                       String mainDir,
                       String outputDir,
                       String dataDir,
    ...
    private void doTask(String queryId) throws IOException {
        ...
        if (entityRankings.containsKey(queryId) && entityQrels.containsKey(queryId)) {
            ...
            if (!results.isEmpty()) {
                documentScore = Utilities.sortByValueDescending(scoreParas(results, documentScore, entityToPseudoDocMap, retEntitySet));
     ...
     * @param entityToPseudoDocMap HashMap where Key = entity and Value = Pseudo-document for this entity
     * @param retEntitySet Set of retrieved entities relevant to the query
    ...
    private Map<String, Float> scoreParas(@NotNull Map<Document, Float> results,
                                          ...
                                          HashMap<String, PseudoDocument> entityToPseudoDocMap, Set<String> retEntitySet) {
        ...
        for (Document doc : results.keySet()) {
            ...
            if (d != null) {
                ...
                float scoreFromECD = results.get(doc);
                ...
                ArrayList<String> pseudoDocEntityList = d.getEntityList();
                Map<String, Integer> freqMap = getDistribution(pseudoDocEntityList, retEntitySet);
                // For every document in this list of documents do
                for (Document document : documentList) {
                    String id = document.get("id");
                    // Get the score of the document
                    int scoreFromEntities = getParaScore(document, freqMap);
                    float totalScore = scoreFromECD + scoreFromEntities;
                    documentScore.put(id, totalScore);
                }
            }
        }
    ...
    private int getParaScore(Document doc, Map<String, Integer> freqMap) {
    ...
    public static void main(@NotNull String[] args) {
        ...
        String outFile = "ECDRetScoreVariation.run";

        new ECDRetScoreVariation(indexDir, mainDir, outputDir, dataDir, paraRunFile, entityRunFile,
